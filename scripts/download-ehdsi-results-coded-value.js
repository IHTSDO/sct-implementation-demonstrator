...
const VALUESET_URL = 'https://fhir.ehdsi.eu/laboratory/ValueSet-eHDSIResultsCodedValueLaboratory.html';
const OUTPUT_FILE = path.join(__dirname, '../src/assets/data/ehdsi-results-coded-value-laboratory.json');
...
function parseValueSetCodes(html) {
  ...
  // The table has structure: <tr><td>Code</td><td>System</td><td>Display</td></tr>
  ...
  for (const table of tables) {
    ...
    while ((match = rowRegex.exec(table)) !== null) {
      ...
      // Expected format: [Code, System, Display, Inactive?, Definition?]
      // We need at least Code, System, and Display
      if (cells.length >= 3) {
        const code = cells[0].trim();
        const system = cells[1].trim();
        const display = cells[2].trim();
        const inactive = cells.length >= 4 ? cells[3].trim().toLowerCase() === 'inactive' : false;
        
        // Only process if we have valid code, system, and display
        if (code && system && display && !inactive) {
          codes.push({
            code: code,
            display: display,
            system: system
...
async function main() {
  try {
    console.log('Downloading eHDSI Results Coded Value Laboratory from:', VALUESET_URL);
    ...
    const output = {
      ...
      description: 'The Value Set is used to code the value of laboratory observation result. It is defined as the union of: (a) eHDSIBloodGroup (b) eHDSIPresenceAbsence (c) eHDSIMicroorganism (d) eHDSIExceptionalValue',
      expansion: {
